...
#include <thorax_truetype/grids.h>
...
GlyphGridBuilder::GlyphGridBuilder(const Shape* shapes, size_t numShapes, size_t firstShape)
...
void GridBuild(TextGrid *grid, const GridRef *input, size_t num_refs, int horizontal_count, int vertical_count)
{
	...
	for (int i = 0; i < num_refs; i++)
	{
		...
		GridPoint lpos = invert(gridref.objectFromParent) * GridPoint(glyph_grid.lowest_x, glyph_grid.lowest_y);
		GridPoint hpos = invert(gridref.objectFromParent) * GridPoint(glyph_grid.highest_x, glyph_grid.highest_y);
	...
	for (int n = 0; n < num_refs; n++)
	{
		...
		GridMatrix2x3 objFrPa_i = invert(gridref.objectFromParent);
